...
import { BarChart3, Plane, Wallet, FileText, Settings, LogOut, Building2, Globe, Users, Compass, Menu, X } from 'lucide-react';
...
export default function AgentLayout({
  ...
  return (
    <div className="flex h-screen bg-background text-foreground flex-col md:flex-row overflow-hidden relative font-sans">
      ...
      {/* Sidebar */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-64 border-r border-border bg-card shadow-sm flex flex-col justify-between
        transform transition-transform duration-300 ease-in-out md:static md:translate-x-0
        ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>
        <div className="flex-1 overflow-y-auto overflow-x-hidden">
          <div className="p-6 border-b border-border/80 hidden md:block">
            <Link href="/" className="flex items-center gap-3 font-black text-lg text-foreground group">
        ...
        <div className="p-4 border-t border-border shrink-0">
          <Link
            href="/login"
            className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl hover:bg-rose-50 text-destructive font-bold transition-colors text-sm"
          >
