...
async function syncFlights() {
  ...
  console.log(`✅ Parsed ${scrapedFlights.length} live flights.`);
  ...
  if (obsoleteFlights.length > 0) {
    ...
    console.log(`🧹 Cleaned up ${toDeleteIds.length} obsolete flights.`);
  }
  ...
  console.log(`🎉 Sync completed in ${(durationMs / 1000).toFixed(1)}s! (${unchangedCount} unchanged, ${updatedCount} updated, ${createdCount} created)`);
}

syncFlights().catch(console.error).finally(() => prisma.$disconnect());
