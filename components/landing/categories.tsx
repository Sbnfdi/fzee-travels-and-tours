        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              { title: "Qatar Groups", desc: "Experience Qatar's modern marvels", img: "https://images.unsplash.com/photo-1582650993077-d9555cb566e2?q=80&w=600&auto=format&fit=crop" },
                ...
                return (
                  <Link href="/login" key={flight.id} className="group bg-card border border-border rounded-2xl overflow-hidden hover:shadow-2xl hover:shadow-primary/10 transition-all duration-300 hover:-translate-y-1 block relative">
                    <div className="h-40 bg-muted overflow-hidden relative">
                      <img src={flightImgs[i % flightImgs.length]} alt={flight.arrivalCity} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                ...
                return (
                  <Link href="/login" key={group.id} className="group bg-card border border-border rounded-2xl overflow-hidden hover:shadow-2xl hover:shadow-primary/10 transition-all duration-300 hover:-translate-y-1 block relative">
                    <div className="h-48 bg-muted overflow-hidden relative">
                      <img src={group.image || groupImgs[i % groupImgs.length]} alt={group.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
        ...
        {hotels.length > 0 && (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {hotels.map((hotel, i) => {
                const hotelImgs = [
                  "https://images.unsplash.com/photo-1542314831-c6a4d14d8c85?q=80&w=600&auto=format&fit=crop",
                  ...
                  "https://images.unsplash.com/photo-1551882547-ff40c0d5b5df?q=80&w=600&auto=format&fit=crop"
                ];
                return (
                  <Link href="/login" key={hotel.id} className="group bg-card border border-border rounded-2xl overflow-hidden hover:shadow-2xl hover:shadow-primary/10 transition-all duration-300 hover:-translate-y-1 block relative">
                    <div className="h-40 bg-muted overflow-hidden relative">
                      <img src={hotel.image || hotelImgs[i % hotelImgs.length]} alt={hotel.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
